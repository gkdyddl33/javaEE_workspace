...
<%
	...
	// 자바 기만의 웹어플리케이션은 어떤 OS건 중립적 실행이 보장되어야 하므로, 특정 시스템에 의존하는 경로를 사용해서는
	// 안된다. 해결책? 개발자가 경로를 넣을려고 하지 말고, 프로그래밍에서 시스템의 경로를 동적으로 얻어와서 이용한다.
	// 디렉토리가 어디있는지 직접 시스템이 찾게 한다는 것!! 스스로 구하게 하는 것!! 개발자가 넣지 말고
	// 이때 사용할 JSP의 내장객체가 바로 application 내장 객체이다!
	// application 내장객체는, 현재 어플리케이션의 전역적 정보를 가진 객체이므로, 어떤 디렉토리에서 사이트가 실행되는지 조차
	// 스스로 알아낼수 있는 능력이 있다..
	String realPath = application.getRealPath("/data");	// 웹사이트의 루트를 기준으로 특정 파일이나, 디렉토리를 명시하면, 스스로 현재 시스템에 
																				// 웹사이트가 얹혀진 os로부터 풀경로를 구해온다. 
	//out.print(realPath);
	
	factory.setRepository(new File(realPath));	// 임시적으로 사용할 경로
	factory.setSizeThreshold(2*1024*1024);	// 용량사이즈
	...
	for(FileItem item : items){
		if(item.isFormField()){// text 입력기반의 컴포넌트라면.. textfiled라면..
